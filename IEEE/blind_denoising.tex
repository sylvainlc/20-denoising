...
\end{align*}
where the last inequality comes from \eqref{eq:def:Y} which states that $Y_s$ is independent of $Y_{\Omega_s}$ given $X_s$. Assuming that $p_{\theta}(\cdot|Y_{\Omega_s})$ is a peaked distribution, we propose to approximate it by the Dirac mass at $\mu_{\theta_d}(Y_{\Omega_s},g(Y_{\Omega_s}))$, i.e. 
$$
