...
More recently, a few models have tried to limit their assumptions on the noise, such as Neighbor2Neighbor \cite{huang2021neighbor2neighbor}, a pair of sub-sampled images is generated from the noisy image. The author then use this pair in a way similar to \cite{lehtinen2018noise2noise}, in order to train their denoising model with one being the input and the other the output. One of the main benefits is that it avoids assumputions on the noise distribution. On the other hand, it relies heavily on the downsampling procedure and does not enable to learn information about the noise distribution.
...
\begin{itemize}
  \item \textit{J-invariance}: The proposed approach is not strictly \textit{J-invariant}, which enables for more flexibility in the network architectures. This also alleviates the need to have several inferences steps such as in \cite{laine2019high}.
...
\begin{itemize}
\item The noise  $(\varepsilon_s)_{s\in \mathsf{S}}$ are independent conditionally to $(X_s)_{s\in \mathsf{S}}$ and the conditional distribution of $\varepsilon_s$ depends on $X_s$ only.
...
\end{align*}
where the last inequality comes from the fact that $Y_s$ is independent of $Y_{\Omega_s}$ given $X_s$. Assuming that $p_{\theta}(\cdot|Y_{\Omega_s})$ is a peaked distribution, we propose to approximate it by the Dirac mass at $\mu_{\theta_d}(Y_{\Omega_s},g(Y_{\Omega_s}))$, i.e.
$$
p_{\theta}(\mathrm{d} x_s|Y_{\Omega_s})\simeq \delta_{\mu_{\theta_d}(Y_{\Omega_s},g(Y_{\Omega_s}))}(\mathrm{d} x_s)\,,
$$
...
$$
where $\tilde \mu^s_{\theta_d} = \mu_{\theta_d}(Y_{\Omega_s},Y_s)$, $\check \mu^s_{\theta_d} = \tilde\mu^s_{\theta_d} +  \mu_{k,\theta_n}(\tilde\mu^s_{\theta_d})$ and $\varphi_{\mu,\sigma}$ is the Gaussian probability density function with mean $\mu$ and standard deviation $\sigma$.
%\begin{multline*}
...
We believe the latter makes a good reference, as it is one of the simplest denoising methods, and it removes noise efficiently but also other high-frequency information such as local contrasts. Training procedure is detailed in \ref{si:baselines}.
...
Note that datasets PN2V-C and PN2V-MA have horizontally-correlated noise (that we take into account: see Section~\ref{sec:datasets}), but significantly lower the performances of methods that are sensitive to it, such as FBI-Denoiser or N2V.
...
We also report evaluation on additional datasets (including a natural images dataset) and compare to other baselines in Table~\ref{si:table:datasetsup}.
...
\label{si:baselines}
\begin{itemize}
  \item N2V and DecoNoisng were trained using the source code provided by the authors, using no positivity constraint, and removing the convolution for N2V: \url{https://github.com/juglab/DecoNoising}
  ...
  \item Neighbor2Neighbor was trained using the code provided by the authors: \url{https://github.com/TaoHuang2018/Neighbor2Neighbor/tree/e66389ebef1e64d306d8fcb95f096cf427243452}. As this method was not trained on fluorescence datasets, we tried to optimise hyperparameters as best as we could on dataset W2S-1 and used the same hyperparameters for all datasets. More precisely we tested $\gamma=1$ and $\gamma=2$, an initial learning rate of $3\cdot10^{-4}$ or $1\cdot10^{-4}$ as suggested by the authors, we increased the number of epochs from 100 to 1000, and tried to decrease the learning rate to $1\cdot10^{-6}$ instead of $2\cdot10^{-5}$ (which surprisingly led to much lower performances). We used $\gamma=1$, an initial learning rate of $3\cdot10^{-4}$ and a minimal learning rate of $2\cdot10^{-5}$.
\end{itemize}
...
Datasets can be downloaded at: \url{https://doi.org/10.5281/zenodo.5156913}, \url{https://doi.org/10.5281/zenodo.5156960} and \url{https://doi.org/10.5281/zenodo.5156937}
