...
Let $\mathsf{S}\subset \mathbb{N}^2$ be a finite set corresponding to the pixel location of an image. In this work, for all $s\in\mathsf{S}$, $X_s$ denotes the signal value at location $s$ and $Y_s$ its noisy observation.  For all $s\in \mathsf{S}$, write %the conditional distribution of $X_s$ given $\{X_u\,;\, u\in \mathsf{S}\,,\, u\neq s\}$ that the observation $Y$ associated with $X$  is given by
%\begin{equation}
%\label{eq:def:Y}
...
\end{multline*}
where $\tilde \mu_{\theta_d} = \mu_{\theta_d}(\Omega_{Y},g(\Omega_{Y}))$. \textcolor{red}{Discuss inference here}.
%
