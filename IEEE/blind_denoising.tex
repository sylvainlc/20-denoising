...
\subsection{Pseudo-loglikelihood}
...
%Assuming that $p_{\theta}(\mathrm{d} x_s|Y_{\Omega_s})$ is a peaked distribution, we propose to approximate it by the Dirac mass at $\mu_{\theta_d}(g(Y_{\Omega_s}),Y_{\Omega_s})$, i.e.
%$$
...
\begin{figure*}[!htbp]
  %\vskip -0.1in
  \begin{center}\includegraphics[width=0.8\textwidth]{fig_plomberie_1col_loss.pdf}
...
The 3 first datasets (\emph{PN2V-C}, \emph{PN2V-MN}, \emph{PN2V-MA}) have been published along with the PN2V and PPN2V methods \cite{krull2019probabilistic, prakash2020fully}, each is composed of several observations of one single FOV.
...
On a NVIDIA RTX-A5000 training time is 97s, 116s and 118s per epoch for N=1, 2 and 3 respectively.
...
PPN2V-GMM refers to fully unsupervised version of the method with bootstrapped GMM noise model as in \cite{prakash2020fully}. PSNR and SSIM values are taken from the article for PN2V-C, PN2V-MN, PN2V-MA. For the other datasets the training do not converge, which was already observed by the authors in \cite{2020DivNoising}.
...
PPN2V-GMM & $36.70$ / $0.9640$ & $36.43$ / $0.9568$ & $33.74$ / $0.8627$ & $NA$ / $NA$ & $NA$ / $NA$ & $NA$ / $NA$\\
...
\label{si:datasetxp}
\paragraph{Datasets published along with the PN2V and PPN2V methods \cite{krull2019probabilistic, prakash2020fully}}
\begin{itemize}
