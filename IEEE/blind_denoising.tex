...
Note that the conditional likelihood of a the central pixel given its neighborhood is given by
$$
p_{\theta}(Y|\Omega_Y) = \int p_{\theta}(x,Y|\Omega_Y) \mathrm{d}x= \int p_{\theta}(x|\Omega_Y)p_{\theta}(Y|x) \mathrm{d}x\,,
$$
where the last inequality comes from \eqref{eq:def:Y} which states that $Y$ is independent of $\Omega_Y$ given $X$. Assuming that $p_{\theta}(\cdot|\Omega_Y)$ is a peaked distribution, it may be approximated by the Dirac mass at $\mu_{\theta_d}(\Omega_Y,g(\Omega_Y))$ which yields the following approximate likelihood:
$$
