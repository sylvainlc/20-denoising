...
In order to push the limits of this trade-off between learning efficiency and denoising quality, we introduced two modifications on the original approach: we used a random dynamic spacing between 3 and 5 pixels, which allow to have relative positions of masked pixels that change randomly. The value of the loss was normalized by the number of points in the grid.
...
We used the same training and evaluation sets as the authors: for each sample type the whole dataset is used for training, and only a subset of the FOV is used for PSNR computation: $(Y,X)\in([0, 512], [0, 512])$ for PN2V-C, $([0, 512], [0, 256])$ for PN2V-MN and $([0, 1024], [0, 512])$ for PN2V-MA.
...
Following the authors, for each FOV, only the raw image of index 249 is used for training or for evaluation, which corresponds better to most use case where only one observation of a given FOV is available.

Images were normalized using the modal value as center and the difference between modal value and $95\%$ percentile as scale factor, computed on the whole dataset. This is relevant in fluorescence microscopy data where signal is often less abundant than background with proportion that vary among images.
...
D-Net and N-Net can be trained jointly, but to take advantage of the fact that D-Net performs better with unmasked input, we introduced an alternate train procedure:
\begin{itemize}
\item D-Net and N-Net are trainned jointly for X epochs
\item The weights of P-Net are fixed and the model is feeded with unmasked inputs for X epochs
\item The weights of P-Net are fixed and the model is trained normaly for X epochs
\end{itemize}
...
To evaluate the capacity of the N-net to capture blindly different noise distributions, we generated 3 synthetic dataset by adding noise to the ground truth of dataset \textit{w2s-0}, so that the datasets have a PSNR value close to the PSNR of the orignial dataset (in a range of $0.1$dB). We used 3 classical noise models:
\begin{itemize}
...
\end{itemize}
Figure \ref{fig:noisestd} show the comparison between the observed and predicted standard deviation of the noise.
...
\caption{Noise estimation. For the observed noise and 3 types of synthetic noises, let $x$ be the ground truth, $y$ the noisy observations (displayed in column \textit{Noisy}) fed to the network and $\hat{x}$ the denoised predicted value (displayed in column \textit{Denoised}). The plots display the observed standard deviation of the noise: $y - x$ as a function of $x$, as well as the predicted standard deviation of the noise $\sigma_n$. The density histogram of the observed values is displayed in blue, in logarithmic scale.}
\label{fig:noisestd}
\end{center}
\vskip -0.2in
\end{figure}

\subsection{Application on real noise}
...
W2S-3 & $20.39$ / $0.2232$ & $34.66$ / $0.9013$ & $34.65$ / $0.8637$ & $35.09$ / $0.9051$ & $36.52$* / $0.9261$* \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
...
\vskip -0.1in
\end{table*}
% footnote text of the 2 footnotes of the caption
\addtocounter{footnote}{-1}
\footnotetext{\label{note:n2v}Using no positivity contraint, and removing the convolution for N2V.}
\addtocounter{footnote}{1}
\footnotetext{\label{note:gaussian}The optimal Gaussian baseline corresponds to the convolution by a gaussian with a standard deviation chosen to maximize the PSNR on the whole evaluation dataset.}
\begin{figure*}[ht]
\vskip 0.2in
\begin{center}
...
\subsection{Improving noise estimation}
