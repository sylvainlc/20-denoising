...
\caption{Noise estimation. For the observed noise and 3 types of synthetic noises, let $X$ be the ground truth and $Y$ the noisy observations (displayed in column \textit{Noisy}). The plots display the observed standard deviation of the noise $Y - X$, as well as the predicted standard deviation of the noise $\sigma_n$. The empirical ditribution of $Y$ is displayed in blue, in logarithmic scale. The empredicted denoised images are displayed in column \textit{Denoised}.}
\label{fig:noisestd}
\end{center}
\vskip -0.2in
\end{figure}
