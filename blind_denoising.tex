...
The 3 first datasets have been published along with the PN2V method\cite{krull2019probabilistic}, each is composed of several observations of a single FOV. \emph{Convallaria} dataset, refered to as \emph{PN2V-C} is composed of 200 images of a $1024x1024$ FOV; \emph{Mouse skull nuclei} referred to as \emph{PN2V-MN} is composed 400 images of a $512x512$ FOV and \emph{Mouse Actin} refered to as \emph{PN2V-MA} is composed of 200 images of a $1024x1024$ FOV. We used the same training and evaluation sets as the authors: for each sample type the whole dataset is used for training, and only a subset of the FOV is used for PSNR computation: $(Y,X)\in([0, 512], [0, 512])$ for PN2V-C, $([0, 512], [0, 256])$ for PN2V-MN and $([0, 1024], [0, 512])$ for PN2V-MA.

The 3 last datasets are the 3 channels of the W2S dataset\cite{zhou2020w2s} refered to as \emph{W2S-1}, \emph{W2S-2} and \emph{W2S-3}. We used the 16-bit raw images kindly provided by the authors. The dataset is composed of 120 FOV of $1024x1024$ pixels, the first 80 are used for training and the last 40 for evaluation. Following the authors, for each FOV, only the raw image of index 249 is used for training and for evaluation.

We scaled the images, using the modal value as center and the difference between modal value and $95\%$ percentile as scale factor, computed on the whole dataset. This is relevant in fluorescence microscopy data where signal is often less abundant than background with proportion that vary among images.
...
% \textit{describe alternate training procedure?}
...
W2S-0 & $21.85$ / $0.3490$ & $33.87$ / $0.9326$ & $34.30$ / $0.9026$ & $34.90$ / $0.9169$ & $35.33$ / $0.9619$ \\
W2S-1 & $19.33$ / $0.2256$ & $32.27$ / $0.8531$ & $31.80$ / $0.8311$ & $32.31$ / $0.8524$ & $33.46$ / $0.8867$ \\
W2S-2 & $20.39$ / $0.2232$ & $34.66$ / $0.9013$ & $34.65$ / $0.8637$ & $35.09$ / $0.9051$ & $36.52$* / $0.9261$* \\
\bottomrule
\end{tabular}
\end{sc}
\end{small}
