...
Image denoising is the process of seperating a noisy image $y = x + n$ into a signal $x$ and a corrupting noise $n$ to remove.
...
Recently, efficient data-driven methods have emerged. Most of them are using pairs of noisy $y$ and clean measurements $x$ of the same signal in a supervised learning problem \cite{weigert2017content}. Lethinen et al. have demonstrated that it is possible to train an efficient denoising method using pairs of independent noisy measurements $(y^1, y^2)$ instead of a clean measurement of the same signal $x$ \cite{lehtinen2018noise2noise}. However clean measurements or pairs of independent noisy measurements of the signal are often unavailable in practice.
...
Current denoising neural network make use of convolutional layers, which have a receptive field, centered on a pixel $y_i$. Through self-supervision, the network predicts $y_i$ given its inputs. Indeed, if the parametrized network is able to use the information of the central pixel $y_i$ to predict it, it would simply learn the identity. A common approach is then to consider a \textit{blind spot} network, either by masking the central pixel, or modifying the convolutional architecture to ensure the central pixel is not used in the computation.

However the majority approches are only \textit{blind spot} during training, as they use the information of the central pixel at test time. They either inject the central pixel by simply removing the mask, but we have little understanding as to why this works well in practice. On the other hand, other approches have tried to model explicitely the contribution of the central pixel at test time, but then need information about the noise, therefore falling under the category of \textit{non-blind denoising}.
...
In practice, the masking procedure can have a large impact on training, as only masked pixels are used for training (typically representing a few percent of the image). In N2N, $M$ is a random sampling of the pixels in the picture, and the masking consists on replacing the pixel by a random neighboring pixel. In N2V, $M$ is a fixed grid, and the pixels are replaced by the average of neighboring pixels (excluding the central pixel).

\textit{charles: jean je te laisse ajouter des choses sur le masking si tu veux déblaterrer la dessus et raconter que le masking est insuffisant? explain the differences and flaws: N2N: no minimal distance between masked pixels + a chance that a pixel is replaced by itself/ N2S: grid replacement: mask also other pixels of the receptive field + replace by donut average.}

Even though the prediction of a noisy pixel conditionnally on its neighbors is an impossible task to learn perfectly, the resulting $f$ shows strong denoising properties. These methods however suffer from high frequency denoising artifacts known as "checkerboard pattern".
...
These works have been extended in deconoising:\cite{goncharova2020, kobayashi2020image} in which a gaussian conlvolution is added after the neural network output to simulate microscope Point Spread Function. These techniques improves performances, however the deconvolved image (predicted image before the gaussian conlolution) has even stronger checkerboard pattern.
...
in the picture. This usually translates in $f$ being a convolutional function which does not depend on the central pixel, but rather on the neighboring ones. This independance guarantees that the function $f$, learnt through self-supervision is not the identity.
...
Among the denoising community, a few works have explicitely modeled the noise process. \cite{laine2019high} for instance model noise process explicitely modeled as gaussian, poissonian or another type of known noise. They either fix the noise parameter(s), or estimate them with a neural network. As they model the signal distribution and the noise distribution, they can include the central pixel's information explicitely at test time in order to improve their performance. This is only possible because they have a truely J-invariant function at test time. Intuitively, the test-time use of central pixel is also happening in other methods such as Noise2Void or Noise2Self, but the dependency on central pixel is not explicit and unknown. Their approach is consistant and yields good performance, but is then restricted to known, synthetic noise.
