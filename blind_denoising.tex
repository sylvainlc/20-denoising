...
\paragraph{D-net.}
The function $\mu_{\theta_d}$ is parametrized by a U-net. The main difference with the networks used in N2S and N2N is that we use upsampling layers with nearest-neighbor approximation instead of transpose convolutions, as we observed that transpose convolution tends to increase the checkerboard artefact. Additional architecture and training information can be found in supplementary section~\ref{si:implementation}.
...
This comment paves the way to many perspectives for our work such as the design of statistically consistent model selection procedures to choose automatically the number of mixing components.
Such approaches have been proposed in more simple cases using for instance Expectation Maximization and penalized maximum likeihood based algorithms.
...
Our method significantly outperforms the 3 baselines both in term of PSNR and SSIM on all datasets.
For the version predicting a simple Gaussian distribution, the average PSNR increase over DecoNoisng is $+1.42$dB.
...
It is worth noting than considering mixture models improves the PSNR in two datasets out of six.
As mentionned in Section~\ref{sec:exp:real:noise} an optimal and data-driven choice of the number of components remains an open (and very challenging) statistical problem but we believe that such experiments support future research in this direction.
